...
export default function Onboarding() {
  ...
  const handleFinish = async () => {
    ...
    try {
      // 1. Upload Logo to Storage if a file was picked
      if (imageFile) {
      ...
      // 2. Update Profile (This stops the redirect loop)
      const { error: updateError } = await supabase
        .from('profiles')
        .update({
          // If they skipped name, we use 'New Vendor' so it's not the default 'My Business'
          ...
          {step === 1 ? (
            ...
            <div className="space-y-6 text-center">
              ...
              <label className="block cursor-pointer">
                <div className="border-2 border-dashed border-zinc-200 rounded-xl p-8 hover:bg-zinc-50 relative overflow-hidden">
                  {previewUrl ? (
