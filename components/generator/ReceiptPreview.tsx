...
export default function ReceiptPreview({ data, settings, receiptRef }: Props) {
  ...
  // Standardize status for consistent logic checks
  ...
  return (
    <div className="flex justify-center items-start font-sans antialiased p-4 relative">
      ...
      >
        {/* PENDING WATERMARK - Shows if currentStatus is pending */}
        {currentStatus === 'pending' && (
            <div className="absolute inset-0 z-50 pointer-events-none flex items-center justify-center overflow-hidden opacity-[0.12] rotate-[-35deg]">
                <h1 className="text-7xl font-black uppercase tracking-tighter border-8 border-zinc-900 px-4 py-2">PENDING</h1>
            </div>
        )}
...
export default function ReceiptPreview({ data, settings, receiptRef }: Props) {
  ...
  return (
    <div className="flex justify-center items-start font-sans antialiased p-4 relative">
      ...
      {/* HARDCODED COLORS HERE PROTECT THE IMAGE EXPORT */}
      <div 
        ref={receiptRef}
        id="receipt-node"
        className="relative text-zinc-900 leading-tight drop-shadow-xl overflow-hidden"
