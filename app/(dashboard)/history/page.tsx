...
import { Plus, Download, Loader2, Eye, X, CheckCircle, Clock, ShieldAlert } from 'lucide-react';
  ...
  return (
    <div className="space-y-8 pb-10">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 px-4 md:px-0">
        ...
        <Link href="/generate" className="bg-brand-orange text-white px-6 py-3.5 rounded-2xl font-black text-[10px] uppercase tracking-widest flex items-center gap-2 shadow-glow-orange active:scale-95 transition-all w-full md:w-auto justify-center">
      ...
      <div className="px-0 md:px-0">
        <div className="bg-white dark:bg-[#0f0f0f] border-y md:border border-slate-200 dark:border-white/10 md:rounded-[32px] overflow-hidden shadow-sm overflow-x-auto custom-scrollbar">
      ...
      {selectedReceipt && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-md">
            <div className="bg-white dark:bg-[#0f0f0f] border border-slate-200 dark:border-white/10 rounded-[40px] shadow-2xl max-w-lg w-full flex flex-col max-h-[90vh]">
                ...
                <div className="p-5 sm:p-8 border-t border-slate-100 dark:border-white/5 bg-slate-50 dark:bg-white/5 rounded-b-[40px]">
                  <div className="flex flex-col sm:flex-row gap-3">

                    {/* LOGIC GUARD: If it's a 6-character order receipt, don't allow marking as paid here! */}
                    {selectedReceipt.status?.toLowerCase() === 'pending' && (
                      selectedReceipt.receipt_number?.length === 6 ? (
                        <button 
                          disabled 
                          className="flex-1 py-3 bg-brand-orange/10 text-brand-orange border border-brand-orange/20 font-black uppercase tracking-widest rounded-2xl flex flex-col items-center justify-center opacity-80"
                        >
                          <span className="text-[10px] flex items-center gap-1.5"><ShieldAlert size={12}/> Storefront Linked</span>
                          <span className="text-[7px]">Update via Orders Page</span>
                        </button>
                      ) : (
                        <button 
                          onClick={() => handleUpdateStatus(selectedReceipt.id)} 
                          disabled={isUpdating} 
                          className="flex-1 py-4 bg-green-500 text-white font-black uppercase text-[10px] tracking-widest rounded-2xl flex items-center justify-center gap-2 hover:bg-green-600 transition-all shadow-lg active:scale-95"
                        >
                    ...
                    <button 
                      ...
                      className="flex-[2] py-4 bg-slate-900 dark:bg-white text-white dark:text-black font-black uppercase text-[10px] tracking-widest rounded-2xl flex items-center justify-center gap-2 hover:opacity-90 transition-all shadow-lg active:scale-95"
                    >
                    ...
                    <button 
                      ...
                      className="flex-1 py-4 bg-white dark:bg-white/10 text-slate-500 dark:text-slate-400 font-black uppercase text-[10px] tracking-widest rounded-2xl transition-all border border-slate-200 dark:border-transparent active:scale-95"
                    >
                      Close
                    </button>
                  </div>
